<script lang="ts">
	...
	let { owner_name, owner_email, language, time_zone } = $derived(data);
			...
			<form
				...
				use:enhance={() => {
					...
					return async ({ result, update }) => {
						...
						if (generalInformationResponseTimeout) clearTimeout(generalInformationResponseTimeout);
				...
				<Card.Footer class="flex gap-4">
					...
					{#if generalInformationFailure}
						<div class={buttonVariants({ variant: 'destructive' })}>
							<X />Uh oh, something went wrong! Please try again later
						</div>
					{/if}
